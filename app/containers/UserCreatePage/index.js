...
import makeSelectUserCreatePage from './selectors';
...
export function UserCreatePage({ dispatch, match }) {
  useInjectReducer({ key: 'userCreatePage', reducer });
  useInjectSaga({ key: 'userCreatePage', saga });
  ...
  const submit = () => {
    if (!loading) {
      ...
      if (!emailRegex.test(email)) {
        ...
        axiosInstance
          .post('api/register-new-account', params)
          .then(() => {
            setLoading(false);
...
const mapStateToProps = createStructuredSelector({
  userCreatePage: makeSelectUserCreatePage(),
});
