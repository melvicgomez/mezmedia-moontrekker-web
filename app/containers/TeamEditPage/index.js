...
import { Row, Select, Table, Input, Spin } from 'antd';
...
import makeSelectTeamEditPage from './selectors';
...
export function TeamEditPage({ dispatch, match }) {
  useInjectReducer({ key: 'teamEditPage', reducer });
  useInjectSaga({ key: 'teamEditPage', saga });
  ...
  const getTeamData = () => {
    axiosInstance
      ...
      .then(res => {
        ...
        setName(teamData.team_name);
        setType(typeList.find(i => i.type === teamData.team_type).name);
        setCorporate(teamData.corporate_id.toString() || '');
        setLoadData(false);
      })
      .catch(() => {
        setLoadData(false);
...
const mapStateToProps = createStructuredSelector({
  teamEditPage: makeSelectTeamEditPage(),
});
